...
C *                                                                     * C
C * THIS ROUTINE COMBINES THE IMAGINARY INTEGRALS TO YIELDS ONLY REAL   * C
C * INTEGRALS.                                                          * C
C *                                                                     * C
C * INPUT :                                                             * C
C *   NORB1 : NUMBER OF ORBITALS DESCRIBING THE CENTER                  * C
C *                                                                     * C
C *   NLMA : IS A ONE-DIMENSIONAL ARRAY CONTAINING THE QUANTUM NUMBERS  * C
C *    OF THE ORBITALS INVOLVED IN THE CALCULATION                      * C
C *                                                                     * C
C *   NTORB : TOTAL NUMBER OF ORBITALS INVOLVED IN THE STUDIED SYSTEM   * C
C *                                                                     * C
C *   NSUM : (NTORB * (NTORB + 1))/2                                    * C
C *                                                                     * C
C *   OUIZA : IS A FOUR-DIMENSIONAL COMPLEX ARRAY CONTAINING THE COMPLEX* C
C *       INTEGRALS                                                     * C
C *                                                                     * C
C * OUTPUT :                                                            * C
C *  FATIMA : IS A ONE-DIMENSIONAL ARRAY CONTAINING THE REAL INTEGRALS  * C
C *********************************************************************** C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC

      SUBROUTINE REAL01(NORB1, NLMA,
     $                                       NTORB, NSUM, OUIZA, FATIMA)
      IMPLICIT REAL*8 (A-H, O-Z)
      COMPLEX*16 OUIZA
      LOGICAL BOOLI0,BOOLI1,BOOLJ0,BOOLJ1, BOOLK0,BOOLK1,BOOLL0,BOOLL1

      DIMENSION NLMA(*), FATIMA(*), OUIZA(20,20,20,20)
