...
C *                                                                     * C
C * THIS ROUTINE RETURNS THE RADIAL INTEGRALS INVOLVED IN THE TWO-CENTER* C
C * OVERLAP INTEGRALS. THE VALUES ARE STORED IN THE ARRAY RADAR.        * C
C *                                                                     * C
C * INPUT :                                                             * C
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      SUBROUTINE ROVELP(LMDL12, N1,L1, N2,L2, ZETA1,ZETA2, AB,
     $                              RLEG01, RLEG02, BESF01, X4NL, RADAR)
      IMPLICIT REAL*8 (A-H, O-Z)
      INCLUDE "../lib95/SIZE.INCL"
      DIMENSION RLEG01(*), RLEG02(*), BESF01(*), X4NL(*), RADAR(*)
      COMMON/STOP00/ZETA1P, N1P, LP2, LP12, ISTR
...
C.... RANGE [AB, +\INFINITY)
      ...
      ISP  = LEG00
      XXXX = DGLGQ(GLROOT, AB, ZETA1+ZETA2, LAG00)
...
C.... COMPUTATION OF THE RADIAL INTEGRALS
      ...
      DO 10 LP12=IABS(L1-LP2), L1+LP2, 2
       ISTR         = 0
       XINT1        = DGLNQ(RAD00, 0.D0, AB, LEG00)
       ISTR         = ISTR + LEG00
       XINT2        = DGLGQ(RAD00, AB, ZETA1+ZETA2, LAG00)
...
C * THE RANGE [0, AB] AND [AB, +\INFTY)                                 * C
C *********************************************************************** C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC

      SUBROUTINE RAD00(T, Y, N)
      IMPLICIT REAL*8 (A-H, O-Z)
      INCLUDE "../lib95/SIZE.INCL"
      DIMENSION T(*), Y(*)
      COMMON/STOP00/ZETA1, N1, LP2, LP12, ISTR
      COMMON/COMG00/GEGIN0(LMHERM*LMHERM*(LMDMAX+L1MAX+L2MAX+1))
